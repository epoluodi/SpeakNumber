...
- (void)FormCompleted:(int)Fromcompleted toComplete:(int)toComplete animated:(BOOL)animated
{
    ...
    if (animated && self.animationDuration > 0.0f)
    {
        CABasicAnimation *animation = [CABasicAnimation animationWithKeyPath:@"completed"];
        animation.duration = self.animationDuration;
        animation.fromValue = [NSNumber numberWithFloat:Fromcompleted];
        animation.toValue = [NSNumber numberWithFloat:toComplete];
        animation.delegate = self;
        [self.layer addAnimation:animation forKey:@"currentAnimation"];
    }
    
    layer.completed = toComplete;
    [layer setNeedsDisplay];
}
